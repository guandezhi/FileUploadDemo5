...
public class FileUploadAdaptorHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    @Override
    protected void messageReceived(final ChannelHandlerContext ctx, HttpObject httpObject) throws Exception {
    ...
    private void writeResponseString(ChannelHandlerContext ctx) {
        ...
        response.headers().setObject(CONTENT_LENGTH, buf.readableBytes());
        ...
        if (!cookies.isEmpty()) {
            for (Cookie cookie : cookies) {
                response.headers().set(HttpHeaderNames.SET_COOKIE, ServerCookieEncoder.encode(cookie));
            }
        }
